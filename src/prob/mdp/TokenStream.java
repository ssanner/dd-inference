...
public class TokenStream {
	...
	while (true) {
	    ...
	    // Handle HTML tags - discard them and insert whitespace in
	    // their place.
	    if (cbuf[0] == '<') {
		if (_bIsHTML) {
		    _bInTag = true;
		...
		cbuf[0] = ' '; // Insert whitespace instead of token
	    } else if (_bInTag) {
		continue; // Discard (text inside tag)
	    }
	    ...
	    // current token if it exists.
			if (!Character.isLetterOrDigit(cbuf[0]) && cbuf[0] != '-' && cbuf[0] != '\'') {
