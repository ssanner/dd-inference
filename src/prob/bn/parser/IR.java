...
public class IR {
	...
	public static class CPT {
		...
		public String toString() {
			return "\n[CPT: " + _sChild + " / " + _alParents + " / Property: "
					+ _sFunProperty + "\n" + _tmEntries;
		}
	}

	public static class CPTComparator implements Comparator {
	...
	public void parse() {
		...
		while ((s = nextToken()) != null && s._nID != Symbol.EOF) {
			...
			if (s._nID == Symbol.SEMI) {
				...
				System.out
						.println("Error while parsing: {net|var|prob} expected, "
								+ s + " found.");
				System.exit(1);
			}
			...
			if (((String) s._value).equalsIgnoreCase("property")) {
				...
				if (s._nID == Symbol.EQUAL) {
					...
					if (s._nID != Symbol.LCBRACE) {
						System.out
								.println("Error while parsing: '=' or '{' expected, "
										+ s + " found.");
						Object o = null;
						o.toString();
						System.exit(1);
				...
				System.out
						.println("Error while parsing: {property|type} expected, "
								+ s + " found.");
				Object o = null;
				o.toString();
				System.exit(1);
	...
	public CPT parseCPT() {
		...
		if (s._nID == Symbol.IDENT) {
			...
			genPTable(cpt._tmEntries, entries.iterator(), cur_assign, vars, 0);

			// Sanity check
			if (cpt._tmEntries.size() != entries.size()) {
				System.out.println("Table: " + cpt);
				System.out
						.println("Error: Table entries did not match expected size");
			...
			while (s._nID != Symbol.RCBRACE) {
				...
				while ((s = nextToken())._nID != Symbol.SEMI) {
					if (s._nID == Symbol.DOUBLE) {
						...
						entries.add(new Double(((Integer) s._value)
								.doubleValue()));
					} else {
						match(s, Symbol.COMMA);
					}
				...
				if (nentries != entries.size()) {
					...
					System.out
							.println("Error: Table entries did not match expected size");
					System.out.println("Expected: " + nentries + ", Actual: "
							+ entries.size());
					System.exit(1);
				}
				...
				for (int i = 0; i < nentries; i++) {
					ArrayList indices = _network.values2Indices(cpt._alParents,
							assign);
	...
	public void genPTable(TreeMap dest, Iterator entries, ArrayList cur_assign,
			ArrayList vars, int var_index) {
		...
		if (s._nID != sym) {
			System.out
					.println("Error while parsing: "
							+ new Symbol(sym, null, -1) + " expected, " + s
							+ " found.");
			Object o = null;
			o.toString();
			System.exit(1);
	...
	public static void main(String[] args) {
		...
		ir.genGraph().genDotFile(file_short + ".dot");
