...
public class DAADDRNode {
    ...
    public DAADDRNode(int lid, double offset, double mult) {
	_nRefID    = lid;
	_dOffset   = offset;
	_dMult     = mult;
    }
    ...
    public String toString(Object context, int depth) {
		...
		sb.append("[ <" + DAADD._df.format(_dOffset) + ","
				+ DAADD._df.format(_dMult) + "> ");

		if (context instanceof DAADD) {
			sb.append(((DAADD) context).getNode(_nRefID).toString(context,
					depth + 1));
		} else {
    ...
    public void toGraph(Object context, Graph g) {
		...
		g.addUniLink("ROOT", "#" + _nRefID, "black", "dashed", 
				"<" + DAADD._df.format(_dOffset) + " + " + DAADD._df.format(_dMult) + " * >");

		if (context instanceof DAADD) {
			((DAADD) context).getNode(_nRefID).toGraph(context, g);
		} else {
