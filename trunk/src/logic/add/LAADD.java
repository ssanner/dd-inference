//////////////////////////////////////////////////////////////////////
//
// Algebraic Decision Diagram Package (Affine ADD - LAADD)
//
...
/**
 * General class for implementation of LAADD data structure
 **/
public class LAADD extends DD {
	...
	// Local data for LAADD 
	public LAADD _this = this; // For inner classes to access
	...
	// An exact count for the LAADD rooted at _pRoot
	...
	public ADDNode getNode(int local_id) {
		if (local_id >= 0 && local_id < _nINodeIDCnt) {
			...
			System.out.println("LAADD.getINode: Error invalid local id: "
	...
	public AADDRNode getRNode(int rid) {
		if (rid >= 0 && rid < _nRNodeIDCnt) {
			...
			System.out.println("LAADD.getRNode: Error invalid local id" + rid
					+ ", >= " + _nRNodeIDCnt);
			try {
				throw new Exception();
			} catch (Exception e) {
	...
	public AADDRNode getINode(int gid, int low, int high, double o_l,
		...
		if (Math.exp(m_l) <= PRECISION) {
			low = 0;
		}
		if (Math.exp(m_h) <= PRECISION) {
			high = 0;
		}
		...
		if (low == high && Math.abs(Math.abs(o_l) - Math.abs(o_h)) <= PRECISION
				&& Math.abs(Math.abs(m_l) - Math.abs(m_h)) <= PRECISION) {
			...
			double r_max = Math.max(logsum(o_l, m_l), logsum(o_h, m_h));
			double r_range = (r_max - r_min);// * 1d/(1 << (Integer)_hmGVarToLevel.get(gid));
			System.out.println(r_min + "  " + r_max + "  " + r_range + " " + o_l + " " + o_h);
			o_l = (logsum(o_l, r_min)) - r_range;
			o_h = (logsum(o_h, r_min)) - r_range;
			m_l = m_l - r_range;
			m_h = m_h - r_range;
	...
	// Produces new LAADD.
	public LAADD scalarMultiply(double val) {
		LAADD a1 = new LAADD(_alOrder);
		AADDRNode aval = a1.getDNode(val, true);
		a1.setRoot(aval);
		return Apply(a1, this, ARITH_PROD);
	}

	// Within current LAADD.
	...
	public LAADD scalarAdd(double val) {
		LAADD new_aadd = new LAADD(this);
	...
	// Within current LAADD, gets *canonical* AADDRNode ref to return
	...
	public PruneResultMap pruneNodeDiff(AADDRNode r, double allow_error) {
		...
		// Collect all nodes in this LAADD and store them in an array of arrays 
		// by var level
		//
	...
	public PruneResult pruneNodeDiffOld(AADDRNode r, double range, boolean recurse, double allow_error) {
		...
		if ((ret._rnode = (AADDRNode) _hmPruneMap.get(_tmpADDRNode)) == null) {
			...
			AADDRNode adiff = applyInt(low2, high2, LAADD.ARITH_MINUS);
			...
			if (/* check already linear! */(low2._nRefID != high2._nRefID)
				...
				case REPLACE_AVG: {
					...
					if (avg_size <= orig_size) {
						...
						//LAADD.MERGE_IDENT_SUBNODES = true;
	...
	// Assume already built with correct order, just needs reduction
	// This also performs restriction (use op = LAADD.RESTRICT_LOW/HIGH).
	...
	public AADDRNode reduceRemap(AADDRNode r, HashMap<Integer,Integer> remap) {
		...
		// Return cached value modified by offset
		return new AADDRNode(ret._nRefID, logsum((r._dMult + ret._dOffset)
	...
	//
	// In reality, a full LAADD package only needs reduceRestrict(), opOut(),
	...
	public AADDRNode reduceRestrict(AADDRNode r, LAADD src, int gid, int op) {
		// Can immediately check for local ID of 0
		if (r._nRefID == 0) {
			return r;
		}
	...
	// Uses root of each LAADD and builds a new LAADD to return.
	public static LAADD Apply(LAADD a1, LAADD a2, int op) {
		// Check for invalid restrict operation
		if (op == LAADD.RESTRICT_LOW || op == LAADD.RESTRICT_HIGH) {
			System.out.println("Cannot RESTRICT using Apply(...)");
			System.exit(1);
		}

		LAADD result = new LAADD(a1._alOrder);
	...
	public AADDRNode apply(AADDRNode a1, LAADD ctxt1, AADDRNode a2, LAADD ctxt2,
			int op) {
		//Compare.ResetTimer();
		if (ctxt1 != this) {
	...
	public AADDRNode buildDDFromUnorderedTreeInt(ArrayList l, Map var2ID) {
		...
		if (o instanceof String && HasOnlyDigits((String) o)) {
			...
			//if (PRINTING_ON)
			//	System.out.println("LAADD BigDecimal: " + printNode(addRNodeRef(dnode)));
			return dnode;
		} else {
			String var = (String) o;
			...
			// Get the var ADD
			AADDRNode high_br = getVarNodeInt(gid, Math.log(0.0d), Math.log(1.0d));
			//if (PRINTING_ON)
			//	System.out.println("LAADD Build 1: " + printNode(addRNodeRef(high_br)));
			high_br = applyInt(high_br, buildDDFromUnorderedTreeInt(
					(ArrayList) l.get(1), var2ID) /*high*/, ARITH_PROD);
			//if (PRINTING_ON)
			//	System.out.println("LAADD Build 2: " + printNode(addRNodeRef(high_br)));

			// Get the !var ADD
			AADDRNode low_br = getVarNodeInt(gid, Math.log(1.0d), Math.log(0.0d));
			//if (PRINTING_ON)
			//	System.out.println("LAADD Build 3: " + printNode(addRNodeRef(low_br)));
			low_br = applyInt(low_br, buildDDFromUnorderedTreeInt((ArrayList) l
					.get(2), var2ID) /*low*/, ARITH_PROD);
			//if (PRINTING_ON)
			//	System.out.println("LAADD Build 4: " + printNode(addRNodeRef(low_br)));

			AADDRNode apply_result = applyInt(low_br, high_br, ARITH_SUM);
			//if (PRINTING_ON)
			//	System.out.println("LAADD Build 5: " + printNode(addRNodeRef(apply_result)));
			return apply_result;
		}
	}
	...
	public AADDRNode buildNode(ArrayList l, Map var2ID) {
		...
		Object o = l.get(0);
		if (o instanceof String && HasOnlyDigits((String) o)) {
			double v = Math.log((new BigInteger((String) o)).doubleValue());
			if (v==Double.NEGATIVE_INFINITY){
			//	v = -50000.0;
			}
			return getDNode(v, true);
		} else if (o instanceof BigDecimal) {
			double v = Math.log(((BigDecimal) o).doubleValue());
			if (v==Double.NEGATIVE_INFINITY){
			//	v = -50000000000000.0;
			}
		...
		//	System.out.println(low._dOffset + " " + low._dMult + " " + high._dOffset + " " + high._dMult);
			//System.out.println("--------------");
	...
	public static LAADD GetConstantAADD(double val, ArrayList order) {
		LAADD a = new LAADD(order);
	...
	/** Build a var ADD **/
	public static LAADD GetVarAADD(int gid, double low, double high,
			ArrayList order) {
		...
		LAADD a = new LAADD(order);
		AADDRNode n = a.getINode(gid, 0, 0, Math.log(low), Math.log(0.0d), Math.log(high), Math.log(0.0d), true);
		a.setRoot(n);
	...
	public int getVarNode(int gid, double low, double high) {
		AADDRNode ret = getINode(gid, 0, 0, Math.log(low), Math.log(0d), Math.log(high), Math.log(0d), true);
		return addRNodeRef(ret);
	}
	...
	public AADDRNode getVarNodeInt(int gid, double low, double high) {
		return getINode(gid, 0, 0, Math.log(low), Math.log(0d), Math.log(high), Math.log(0d), true);
	}
	...
	public AADDRNode getConstantNodeInt(double val) {
	  ...
	  public static double logsum(double log_a, double log_b) { 		
  		if (log_a == Double.NEGATIVE_INFINITY){
  			if(log_b == Double.NEGATIVE_INFINITY){
  			return Double.NEGATIVE_INFINITY;
  			}
  			else 
  				return log_b;
  }
  		else if (log_b == Double.NEGATIVE_INFINITY)
  			return log_a;
